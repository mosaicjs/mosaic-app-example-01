...
export default class AppMenuPanel extends React.Component {
    ...
    _renderLocaleMenu() {
        ...
        return (
            <DropdownButton eventKey={list[0]} title={messages.getCurrentLanguage(app.state.locale)}>
    ...
    _renderScreenReferences(){
        const messages = this._getMessages();
        return [
            <NavItem onClick={this._showScreen.bind(this, 'tags/')} eventKey={1} href='#'>{messages.screenTags}</NavItem>,
            <NavItem onClick={this._showScreen.bind(this, '')} eventKey={2} href='#'>{messages.screenOrganizations}</NavItem>
        ];
    }
    render(){
       const app = this.props.app;
